...
function loadAuth() {
  ...
  if (fs.existsSync(AUTH_FILE)) {
    try {
      ...
      if (parsed && parsed.email && parsed.passwordHash && !parsed.users) {
        return { users: [{ id: 'u-1', email: parsed.email, passwordHash: parsed.passwordHash, isAdmin: true, createdAt: new Date().toISOString() }], sessionSecret: parsed.sessionSecret };
      }
